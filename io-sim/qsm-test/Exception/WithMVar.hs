...
incr :: (MonadSay m, MonadCatch m, MonadMVar m) => AtomicCounter m -> m ()
incr (AtomicCounter ref) =
  fst . fst <$> generalBracket
    (takeMVar ref)
    (\originalValue -> \case
        ExitCaseSuccess (_, a) -> say "CaseSuccess" >> putMVar ref a
        _ -> say "CaseFail" >> putMVar ref originalValue
    )
    (\x -> pure ((), x + 1))

semantics :: (MonadSay m, MonadCatch m, MonadMVar m) => AtomicCounter m -> Cmd Concrete -> m (Resp Concrete)
