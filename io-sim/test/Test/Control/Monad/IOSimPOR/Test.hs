...
module Test.Control.Monad.IOSimPOR.Test (tests) where
...
import           Test.Control.Monad.Common
import           Test.STM
...
shrinkDelays tasks
  | null times = []
  | otherwise  = [[(Task . removeTime d) steps | Task steps <- tasks]
                 | d <- times]
  ...
  where interpretStep _ (WhenSet m n) = atomically $ do
        ...
        interpretStep (_,timer) (Timeout tstep) = do
          ...
          case (timerVal,tstep) of
            ...
            (Just tout,AwaitTimeout)    -> atomically $ void (awaitTimeout tout)
            (Nothing,_)                 -> return ()
...
propPermutations :: Small Int -> Property
propPermutations (Small n) =
  traceNoDuplicates $ \addTrace ->
